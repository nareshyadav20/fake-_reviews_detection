...
def predict():
    ...
    if fake_score > genuine_score:
        ...
        # Generate detailed, specific conclusion based on actual detected features
        reasons = []
        ...
        if reasons:
            ...
            conclusion = f"This review appears fake due to its overly enthusiastic tone and lack of specific details, which are typical indicators of fabricated reviews designed to appear overly positive."
            
    elif fake_score == genuine_score:
        # Tie-breaker: if promotional words or excessive exclamation, classify as fake
        if features['promotional_count'] >= 1 or features['exclamation_count'] >= 2:
            classification = "Likely Fake"
            confidence = 55
            conclusion = f"This review appears fake because it contains promotional language or excessive punctuation, which are indicators of fabricated content despite other balanced characteristics."
        else:
            classification = "Likely Genuine"
            confidence = 55
            conclusion = f"This review appears genuine due to its balanced characteristics and lack of strong fake indicators."
    else:
        classification = "Likely Genuine"
        confidence = min(95, 65 + (genuine_score * 4))
        
        # Generate detailed, specific conclusion for genuine reviews
        reasons = []
        ...
        if reasons:
            ...
            conclusion = f"This review appears genuine due to its natural language patterns and balanced tone, which are consistent with authentic customer feedback rather than promotional content."
